...
idt_size=$-idt
	...
	surname db         "Sukhachev "                  	;Без пробела съедает последний символ
	real_sp dw         0                             	;ячейка для хранения SP
	real_ss dw         0                             	;ячейка для хранения SS
	pos     dw         950
	mes     db         "Hello World!",27,'[0m$'
...
main proc
	...
	;инициализируем ES и выводим символы
	          ...
	          mov    ES,AX
	          mov    BX,3266                    	;начальное смещение на экране
	          mov    CX,9                       	;число выводимых символов

	          mov    si,0
	screen:   
	          mov    AX,word ptr surname[si]
	          mov    ES:[BX], AX                	;вывод в видеобуфер
	          add    BX, 2                      	;2	;сместимся в видеобуфере
